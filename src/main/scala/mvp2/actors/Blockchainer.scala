...
import mvp2.messages.{CurrentBlockchainInfo, CurrentState, Get}
...
class Blockchainer(settings: Settings) extends PersistentActor with Blockchain with StrictLogging {
  ...
  override def receiveRecover: Receive = {
    case keyBlock: KeyBlock =>
      println(s"Updated KeyBlock: ${keyBlock.height}")
      update(keyBlock)
  ...
  override def receiveCommand: Receive = {
    ...
    case Get =>
      println(s"request for a chain")
      chain.foreach(x => println(x._2 + " from chain"))
      val currentState: Future[CurrentState] = (accountant ? Get)(10.seconds).mapTo[CurrentState]
      currentState.onComplete { state =>
        sender() ! CurrentState(chain, state.get.accInfo, state.get.stateRoot)
      }
    case _ => logger.info("Got something strange at Blockchainer!")
  }

  def saveModifier(block: Block): Unit = {
    println(s"${block.isValid} -> ${block.height}")
    if (block.isValid) block match {
      case keyBlock: KeyBlock =>
        logger.info(s"New keyBlock with height ${keyBlock.height} is received on blockchainer.")
        appendix.chain.foreach(block =>
          persist(block._2) { x =>
            println(s"Successfully saved block with id: ${x.currentBlockHash} and height ${x.height}!")
          })
